...
*/
public class ModificarTalleSCR extends javax.swing.JFrame {
	
	/**
	 * 
	 */
	private static final long serialVersionUID = 8216211246248849765L;
	...
	*/
	public static void main(String[] args) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				ModificarTalleSCR inst = new ModificarTalleSCR();
...
//				List<TalleDTO>  listatdto = BusinessDelegate.getInstancia().getAllTalle();
				if(listatdto != null){
					...
					ListModel<String> jListTalleesModel = new DefaultComboBoxModel<String>(descs);
					jListTalles = new JList();
					jListTalles.setModel(jListTalleesModel);
				}
				
			}
			{
				jLabelIndicacion = new JLabel();
				jLabelIndicacion.setText("Elija el Talle a modificar y pulse Modificar:");
			}
			{
					...
					public void actionPerformed(ActionEvent arg0) {
						...
						for(TalleDTO busdto : listatdto){
							if(busdto.getDescripcion().equals(val)){
										...
										editadodto.setIdTalle(busdto.getIdTalle());
					...
					public void actionPerformed(ActionEvent arg0) {
								...
								try {
									editadodto.setDescripcion(jTextFieldNuevaDescripcion.getText().toUpperCase());
									 BusinessDelegate.getInstancia().modificarTalle(editadodto);
										jLabelMensaje.setText("Se actualizó el Talle "+ editadodto.getDescripcion());
										jButtonAceptar.setEnabled(false);
										jTextFieldNuevaDescripcion.setEnabled(false);
