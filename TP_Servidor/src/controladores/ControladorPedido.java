...
public class ControladorPedido {
	...
	private void CrearOrdenProduccionParcial(ItemFaltantePedido itemFaltantePedido) {
		//VerificarDisponibilidaddeMateriaPrima
		List<ItemFaltantePedido> lstItemsFaltantesPedido = new ArrayList<ItemFaltantePedido>();
		...
		for(ItemMaterialPrenda itt:ipr.getItemMaterialPrenda()){			
			...
			float cantidadFaltante = ((itt.getDespedicio()+itt.getDespedicio())*(itemFaltantePedido.getCantidadFaltante())) - Almacen.getInstancia().CalcularDisponibleEnMP(lstItemBultoMP);				
			//hacer un metodo que reciba una lista de bultos, y cantidad - y en base a eso vaya insertando en la tabla de 					
			//Reservas que bultos tiene reservado que cantidad y para que pedido en particular	


			///ARREGLAR RESERVAS MP
			//Almacen.getInstancia().ReservarItemsMP(lstItemBultoMP, itemFaltantePedido.getCantidadFaltante(), itemFaltantePedido.getPedido(),ItemPedido i);																	
		...
		if (lstItemsFaltantesPedido.size()==0)
		{
			...
			ControladorProduccion.getInstancia().CrearOrden(or);
